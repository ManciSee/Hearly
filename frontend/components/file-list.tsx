...
export default function FileList() {
  ...
  const [transcriptions, setTranscriptions] = useState<Record<string, TranscriptionStatus>>({})
  ...
  const pollTranscriptionStatus = async (id: string, token: string) => {
    try {
      const response = await axios.get(`http://localhost:8000/transcription/${id}?check_status=true`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
      ...
      if (data.status !== "COMPLETED" && data.status !== "ERROR" && data.status !== "FAILED") {
        ...
        setFiles((prev) => prev.map((file) => (file.id === id ? { ...file, status: data.status } : file)))
      }
    } catch (err) {
  ...
  const fetchTranscription = async (id: string) => {
    try {
      ...
      const response = await axios.get(`http://localhost:8000/transcription/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  const toggleTranscription = (id: string) => {
    if (selectedId === id) {
      ...
      const transcriptionStatus = transcriptions[id]?.status

      if (fileStatus === "PENDING" && !transcriptionStatus) {
        ...
        // Se la trascrizione è già completata, mostra/nascondi
        setSelectedId(id)
      } else if (fileStatus !== "PENDING") {
        // Se il file non è in PENDING ma non abbiamo la trascrizione in memoria, recuperala
  ...
  const getSummary = async (id: string) => {
    try {
      ...
      const response = await axios.get(`http://localhost:8000/summarize/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  return (
    <Card className="w-full">
      ...
      <CardContent>
        {error && <div className="text-center text-red-600 py-4 mb-4">{error}</div>}

        {files.length === 0 ? (
          <div className="text-center py-8">
            <p className="text-gray-500">Non hai ancora trascrizioni. Carica un file audio per iniziare.</p>
          </div>
        ) : (
          <div className="space-y-4">
            {files.map((file) => {
              const fileStatus = file.status || "PENDING"
              const isTranscribed = fileStatus !== "PENDING" || transcriptions[file.id]?.status === "COMPLETED"
              const isProcessing = loadingFiles[file.id] || transcriptions[file.id]?.status === "QUEUED"

              return (
                <div key={file.id} className="border rounded-lg p-4 shadow-sm">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div className="flex-1">
                      <h3 className="font-medium text-gray-700">{file.filename}</h3>
                      <div className="flex items-center gap-2">
                        <audio controls src={file.url} className="mt-2 w-full" />
                        {fileStatus !== "PENDING" && (
                          <span className="text-xs px-2 py-1 rounded-full bg-green-100 text-green-800">Trascritto</span>
                        )}
                      </div>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {fileStatus === "PENDING" && !isProcessing ? (
                        ...
                        <Button onClick={() => toggleTranscription(file.id)} disabled={isProcessing}>
                    ...
                    <div className="mt-4 space-y-4">
                      <div className="p-4 bg-gray-50 border rounded-md text-sm text-gray-800 whitespace-pre-wrap">
                        ...
                        {transcriptions[file.id]?.status === "COMPLETED" ? (
                          ...
                          <p className="text-red-600">Errore durante la trascrizione. Riprova!</p>
                        ) : (
                          <div className="flex items-center">
                            ...
                            <span>
                              ...
                              {transcriptions[file.id]?.message && (
                                <span className="text-xs text-gray-500 ml-1">({transcriptions[file.id]?.message})</span>
                              )}
                            </span>
                          </div>
                        )}
